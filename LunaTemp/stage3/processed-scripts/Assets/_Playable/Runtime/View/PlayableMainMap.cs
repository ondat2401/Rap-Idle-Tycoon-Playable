...
namespace _Playable.Runtime.View
{
    ...
    public sealed class PlayableMainMap : MonoBehaviour
    {
        ...
        /// </summary>
        public IEnumerator SetLevel(PlayableMapSlot slot, int level, [Bridge.Ref] PlayableTiming timing)
        {
            if (!this.TryResolve(slot, level, out SpriteRenderer renderer, out Sprite sprite))
            {
                yield break;
        ...
        /// </summary>
        public IEnumerator SetLevelAll(int level, [Bridge.Ref] PlayableTiming timing, params PlayableMapSlot[] except)
        {
...
[Bridge.Ref]             Vector3 localPosition)
        {
            this.DestroyClone(slot);
